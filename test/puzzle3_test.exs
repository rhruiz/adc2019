defmodule Puzzle3Test do
  ...
  describe "path_to_points/1" do
    test "drive around the block" do
      ...
      assert MapSet.member?(points, {1, 0})
      assert MapSet.member?(points, {1, 1})
      assert MapSet.member?(points, {0, 1})
      assert MapSet.member?(points, {0, 0})
    end
    ...
    test "generates all the points in the segment" do
      ...
      assert MapSet.member?(points, {1, 0})
      assert MapSet.member?(points, {2, 0})
      assert MapSet.member?(points, {3, 0})
    end
  end
